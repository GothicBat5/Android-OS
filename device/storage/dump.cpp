...
void read_buffer(int buf_id, int total_len, const char* path)
{
    ...
    for (i=0;i<=total_len;i+=524288) 
    {
        ...
        len = 524288;
        std::cout("/vendor/bin/sg_read_buffer -m 0x1c -i %d -l %d -o %d -r /dev/sg3 >> /data/vendor/storage/%s\n",
            buf_id, len, i, path);
        auto cmd = android::base::StringPrintf("/vendor/bin/sg_read_buffer -m 0x1c -i %d -l %d -o %d -r /dev/sg3 >> /data/vendor/storage/%s\n",
            buf_id, len, i, path);
        system(cmd.c_str());
    }
}
...
int main() 
{
    ...
    dumpFileContent("UFS Slow IO Write", "/dev/sys/block/bootdevice/slowio_write_cnt");
    dumpFileContent("UFS Slow IO Unmap", "/dev/sys/block/bootdevice/slowio_unmap_cnt");
    dumpFileContent("UFS Slow IO Sync",  "/dev/sys/block/bootdevice/slowio_sync_cnt");
    printf("------ UFS err_stats ------\n");
  
    const std::string ufs_err_stats_path("/dev/sys/block/bootdevice/err_stats/");
    std::unique_ptr<DIR, decltype(&closedir)> statdir(opendir(ufs_err_stats_path.c_str()), closedir);
    ...
    dumpFileContent("", "/dev/sys/block/bootdevice/health_descriptor/life_time_estimation_c");
    printf("\n------ UFS error history ------\n");
